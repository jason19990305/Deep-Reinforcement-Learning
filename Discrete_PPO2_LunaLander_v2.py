...
actor_output = Dense(num_actions,activation='softmax',kernel_initializer=Orthogonal(gain=0.01) ,bias_initializer=Zeros())(actor_dense3)
#Critic
...
critic_output = Dense(1,activation='linear' ,kernel_initializer=Orthogonal(gain=np.sqrt(2)) ,bias_initializer=Zeros())(actor_dense3)
...
ppo2 = Discrete_PPO2.PPO2(state_space=num_states,
                ...
                iteration=10,
                warm_up=5,# Actor stop training for warm_up times
...
total_reward_list,avg_reward_list = ppo2_agent.fit(env,episodes=5000)
